...
export class CommandRepository {
  ...
  public listSince(since: string): PlaybackCommand[] {
    const rows = this.db
      .prepare("SELECT * FROM playback_commands WHERE issued_at >= ? ORDER BY issued_at DESC")
      .all(since) as unknown as CommandRow[];
